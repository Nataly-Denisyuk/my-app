...
import ordersQueryData from "../queries/fakedata/ordersQueryData";
...
export default class FakeQueriesApi {
  ...
  execute(query) {
    ...
    if (query instanceof StatusesQuery)
      return this._getWithTimeout(statusesQueryData);

    // orders (static json)
    if (query instanceof OrdersQuery)
      return this._getWithTimeout(ordersQueryData);

    // instruments (func)
    if (query instanceof InstrumentsQuery)
      return this._getWithTimeout(() =>
        instrumentsQueryData(query.params.filter)
      );

    return new Promise((_, reject) => {
      reject(new Error(`${query.url} is not supported by FakeQueriesApi`));
    });
  }
    ...
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        try {
          ...
          let result =
            typeof resultFuncOrObj === "function"
              ? resultFuncOrObj()
              : resultFuncOrObj;
