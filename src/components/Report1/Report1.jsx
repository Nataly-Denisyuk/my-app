...
function Report1({api}) {
  ...
  useEffect(() => {
    (async () => {
      if (instrumentsState.searchText.trim() !== ''){
        ...
        try{        
          const instruments = await api.query.execute(new InstrumentsQuery(instrumentsState.searchText))        
          setInstrumentsState(state => ({...state, isLoading: false, options: instruments.map(i => ({ value: i.id.toString(), text: i.name })) }))
        }
        catch(e){
          setInstrumentsState(state => ({...state, isLoading: false, options: []}))
  ...
  const fetchData = useCallback(async () => {
    ...
    try{
      ...
      setState(state => ({...state, loading: false, error: null, data: { statuses }}))
    }
    catch(e){
  ...
  const statusesOptions = [
    { value: "0", text: "--- Любой ---"},
    ...state.data.statuses.map(s => ({ value: s.id.toString(), text: s.statusName}))
  ]  

  return (
    <DisplayContainer>
      <Field label="Статус">
        <SelectField value={filterState.status} onChange={el => setFilterState(prevState => ({...prevState, status: el.target.value}))}>
          {
            statusesOptions.map(o => <Option key={o.value} value={o.value}>{o.text}</Option>)
          }
        </SelectField>
      </Field>      
      <Field label="Инструмент">
        <SelectField 
          ...
          onChange={el => setInstrumentsState(prevState => ({...prevState, value: el.target.value}))}
        ...
        >
          {
            instrumentsState.options.map(o => <Option key={o.value} value={o.value}>{o.text}</Option>)
          }
        </SelectField>
      </Field>      
    </DisplayContainer>
